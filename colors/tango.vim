...
" Vim Color File
" Name:       tango.vim
" Maintainer: https://github.com/lePerdu
...
" < https://github.com/lePerdu/tango.vim/blob/master/README.md >
" for more information.)
if !exists("g:tango_termcolors")
  let g:tango_termcolors = 256
endif

" Not all terminals support italics properly. If yours does, opt-in.
if !exists("g:tango_terminal_italics")
  let g:tango_terminal_italics = 0
endif

if !exists("g:tango_disable_background")
  let g:tango_disable_background = 0
endif

" This function is based on one from onedark.vim: https://github.com/joshdick/onedark.vim
let s:group_colors = {} " Cache of default highlight group settings, for later reference via `tango#extend_highlight`
function! s:h(group, style, ...)
  if (a:0 > 0) " Will be true if we got here from tango#extend_highlight
...
function! tango#set_highlight(group, style)
  call s:h(a:group, a:style)
endfunction

function! tango#extend_highlight(group, style)
...
let s:gutter_fg = s:colors.gutter_fg
let s:cursor_grey = s:colors.cursor_grey
let s:visual_grey = s:colors.visual_grey
let s:menu_grey = s:colors.menu_grey
let s:special_brown = s:colors.special_brown
let s:vertsplit = s:colors.vertsplit
let s:trans_black = extend(copy(s:black), {"gui": "NONE"})
...
call s:h("LineNr", { "fg": s:gutter_fg }) " Line number for ":number" and ":#" commands, and when 'number' or 'relativenumber' option is set.
call s:h("CursorLineNr", {}) " Like LineNr when 'cursorline' or 'relativenumber' is set for the cursor line.
call s:h("MatchParen", { "fg": s:blue, "gui": "underline" }) " The character under the cursor or just before it, if it is a paired bracket, and its match.
call s:h("ModeMsg", {}) " 'showmode' message (e.g., "-- INSERT --")
call s:h("MoreMsg", {}) " more-prompt
call s:h("NonText", { "fg": s:special_brown }) " '~' and '@' at the end of the window, characters from 'showbreak' and other characters that do not really exist in the text (e.g., ">" displayed when a double-wide character doesn't fit at the end of the line).
...
call s:h("PmenuSbar", { "bg": s:special_brown }) " Popup menu: scrollbar.
...
call s:h("SpecialKey", { "fg": s:special_brown }) " Meta and special keys listed with ":map", also for text used to show unprintable characters in the text, 'listchars'. Generally: text that is displayed differently from what it really is.
